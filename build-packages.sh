...
BUILD="1"
JAR="./dist/jicmp2-api-$VERSION.jar"
LIB="./dist/libjicmp2.so"
...
function createPackage() {
  ...
  fpm -s dir \
      --name jrrd2 \
      --description "A native ICMP interface for Java" \
      --vendor "OpenNMS" \
      --license "GPLv2" \
      --maintainer "opennms@opennms.org" \
      --url "https://github.com/OpenNMS/jicmp2" \
      --version $VERSION \
      -t $TYPE \
      -C ./tmp/$TYPE \
...
cp $LIB ./tmp/rpm/usr/lib64/libjicmp2.so
chmod 755 ./tmp/rpm/usr/lib64/libjicmp2.so
cp $JAR ./tmp/rpm/usr/share/java/jicmp2.jar
chmod 644 ./tmp/rpm/usr/share/java/jicmp2.jar
...
cp $LIB ./tmp/deb/usr/lib/jni/libjicmp2.so
chmod 755 ./tmp/deb/usr/lib/jni/libjicmp2.so
cp $JAR ./tmp/deb/usr/share/java/jicmp2.jar
chmod 644 ./tmp/deb/usr/share/java/jicmp2.jar
